...
export default function ProductWeightIndex() {
  ...
  const { data: data = [], isLoading } = useGet<ProductWeight[]>({
    queryKey: ["productweights"],
    queryFn: productWeightService.getAll,
  });

  const { data: products = [] } = useGet<any[]>({
    queryKey: ["products"],
    queryFn: productService.getAll,
  });

  const { data: criterias = [] } = useGet<any[]>({
    queryKey: ["criterias"],
    queryFn: criteriaService.getAll,
  });

  const { data: subCriteriaList = [] } = useGet<any[]>({
  ...
  const filteredData = useMemo(() => {
    if (selectedProductFilter === "ALL") return data;
  ...
  return (
    <div className="space-y-6 pb-12">
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                            isSelected
                              ? "border-gray-900 bg-gray-900 dark:border-white dark:bg-white text-white dark:text-gray-900"
      ...
      <ModalConfirm
        ...
        message={
          <span>
            Apakah kamu yakin ingin menghapus bobot spesifikasi{" "}
            <strong className="font-bold text-gray-900 dark:text-white">{deleteTarget?.name}</strong>? Data perhitungan SPK yang terkait dengan spesifikasi ini akan berubah.
          </span>
        }
