...
export default function AddProductWeight() {
  ...
  const { data: products = [], isLoading: isProductsLoading } = useGet({
    queryKey: ["products"],
    queryFn: productService.getAll,
  });

  const { data: criterias = [], isLoading: isCriteriasLoading } = useGet({
    queryKey: ["criterias"],
    queryFn: criteriaService.getAll,
  });

  const { data: allSubCriterias = [], isLoading: isSubCriteriasLoading } = useGet({
    queryKey: ["subcriterias"],
    queryFn: subCriteriaService.getAll,
  });

  const { data: existingWeights = [], isLoading: isWeightsLoading } = useGet({
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className={`w-full border border-gray-300 rounded-xl px-3.5 py-3 outline-none transition-all font-semibold text-sm bg-white text-gray-900 focus:ring-2 focus:ring-black cursor-pointer ${
                  !selectedProductId ? "border-black" : ""
                }`}
              >
                <option value={0}>
                  {isDataLoading
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-gray-200/60 text-xs">
                            ...
                            <span
                              className={`font-mono font-bold px-2.5 py-1 rounded-lg transition-all ${
                                currentSelectedSub > 0
