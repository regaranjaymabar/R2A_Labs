...
import { ArrowLeft, CheckCircle2, Ban, DollarSign, Boxes, Package } from "lucide-react";
...
export default function EditStock() {
  ...
  useEffect(() => {
    if (item) {
      ...
      setIsAvailable(Boolean(item.isAvailable ?? item.is_available ?? true));
    }
  }, [item]);
          ...
          <div className="p-4 bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-gray-800 flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-base font-bold text-gray-900 dark:text-white">
                {(item as any).product?.modelName || (item as any).product_name || `Laptop ID #${item.productId || item.product_id}`}
              </span>
            </div>
          </div>
        )}
